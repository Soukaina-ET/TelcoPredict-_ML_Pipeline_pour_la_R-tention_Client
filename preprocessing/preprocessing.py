...
def get_pipeline():
    """Construction de la Pipeline de transformation optimisée"""
    
    # 1. Variables numériques (on ajoute les nouvelles variables)
    num_features = ['tenure', 'MonthlyCharges', 'TotalCharges', 'Charges_per_Tenure', 'Total_Services', 'Is_Long_Term']
    
    # 2. Variables catégorielles (on élargit pour donner plus de données au XGBoost)
    cat_features = [
        'Contract', 'InternetService', 'PaymentMethod', 'OnlineSecurity', 
        'OnlineBackup', 'DeviceProtection', 'TechSupport', 'StreamingTV', 'StreamingMovies'
    ]

    # Pipeline Numérique : Scaling
    ...
    preprocessor = ColumnTransformer(
        ...
        remainder='drop' # Supprime les colonnes non listées (comme customerID)
    )
